"""
Table-field extractor built on the Claude Agent SDK (`claude-agent-sdk`, Python).

Unit of work = ONE (paper, table field). One `query()` call = one session.

WHY THIS SHAPE
--------------
* `output_format={"type":"json_schema", ...}` is a field on ClaudeAgentOptions and
  cannot be swapped mid-session (there is no `set_output_format()` on
  ClaudeSDKClient). Every table field needs its own schema, so a persistent client
  cannot serve more than one field. => `query()` per (paper, field).
* The paper markdown is INLINED into the prompt ahead of the field spec, so the
  cacheable prefix is `system | paper | field-spec`. Sibling field-sessions on the
  same paper therefore share the `system | paper` prefix (cache read @0.1x instead
  of a fresh cache write @1.25x). Inlining also removes the Read round-trip that a
  file-based design pays before it can reason at all.
* NO row-fanout subagents. A subagent starts with a fresh context and cannot
  inherit the parent's cached paper, so N rows => N x cache-write of a 16k-token
  paper. Within one session the paper is resident once and extra reasoning turns
  cost only a cache read of the prefix.
* Full-cell quote verification runs in PYTHON (post-hoc, free, deterministic) using
  the repo's own `utils.source_linker`. The in-loop `verify_quotes` MCP tool checks
  only a bounded sample, to catch a systematic quoting error before it is baked
  into ~180 cells. Cells that fail the post-hoc sweep are repaired with
  `query(resume=session_id)`, which re-uses the already-cached prefix.

OUTPUT CONTRACT (must match backend/dspy_components/runtime_builders.py)
-----------------------------------------------------------------------
    {"<field_name>": {"value": [ {col: {"value": ..., "source_text": ...}, ...} ],
                      "source_text": "<table caption / lead-in sentence>",
                      "status": "extracted"}}
Empty table -> {"value": "NR", "source_text": "NR", "status": "not_reported"},
exactly as `_TwoStageExtractor` emits when Stage 1 finds no rows.

INSTALL
-------
    pip install claude-agent-sdk          # not currently installed on this box
    export ANTHROPIC_API_KEY=...          # or CLAUDE_CODE_USE_BEDROCK=1 + AWS creds

USAGE
-----
    python extract_table_agent.py --dry-run
    python extract_table_agent.py --schema .../dynamic_6c8eecce_ContinuousOutcomesV2.json \
                                  --papers "Artese 2015" "Chen 2012"
    python extract_table_agent.py --schema .../oral_cancer_index_test.json --limit 5
"""
...
try:
    from utils.source_linker import (  # type: ignore
        build_source_index,
        locate_source,
        locate_value,
...
try:
    from dspy_components.runtime_builders import _compose_field_desc  # type: ignore
    _HAVE_COMPOSER = True
except Exception:
...
class Config:
    ...
    effort_small: str = "medium"          # tables expected <= effort_row_switch rows
    effort_large: str = "high"            # taller tables: attribution errors dominate
    effort_row_switch: int = 6
    ...
    budget_usd_base: float = 0.45         # covers a ~9-col x 20-row table + repair
    budget_usd_per_row: float = 0.012
    budget_usd_cap: float = 1.20
    ...
    use_row_census_subagent: bool = False  # fresh-context independent row count
    census_model: str = "sonnet"
    census_effort: str = "medium"
...
# ═════════════════════════════════════════════════════════════════════════════
_NR = "NR"
_STATUS_VALUES = ["extracted", "not_reported", "partial"]
...
def build_output_schema(
    ...
    Two modes for the not-reported case:
      * default (`strict_nr_union=False`): `value` is always an array. An empty
        ...
        same conversion `_TwoStageExtractor` does when Stage 1 yields no rows.
...
def validate_envelope(
    ...
    if outer.get("status") not in _STATUS_VALUES:
...
def normalize_envelope(env: Dict[str, Any], field_name: str) -> Dict[str, Any]:
    ...
    if isinstance(outer, dict) and outer.get("value") == []:
        return {field_name: {"value": _NR, "source_text": _NR, "status": "not_reported"}}
    if isinstance(outer, dict) and outer.get("value") == _NR:
        outer.setdefault("source_text", _NR)
        outer["status"] = "not_reported"
    return env
...
def _row_key(row: Dict[str, Any], anchors: List[str]) -> str:
    """Canonical row identity, computed on anchor columns only.
...
def missing_plan_rows(
    ...
    Returns [] when the field has no anchor columns - identities can't be matched
...
def row_census_agent(cfg: Config) -> AgentDefinition:
    ...
    return AgentDefinition(
        ...
        tools=["Read", "Grep"],
        model=cfg.census_model,
        effort=cfg.census_effort,
        maxTurns=8,
    )
...
def build_options(
    *,
    ...
    budget = min(
        cfg.budget_usd_cap, cfg.budget_usd_base + expected_rows * cfg.budget_usd_per_row
    )
    ...
    return ClaudeAgentOptions(
        ...
        effort=(cfg.effort_large if expected_rows > cfg.effort_row_switch
...
async def extract_table(
    *,
    ...
    exp_rows = expected_rows if expected_rows is not None else (
        8 if n_cols <= 10 else 4
    )
    ...
    trace_path = None
    if cfg.trace:
        tdir = trace_dir or (OUT_DIR / "traces")
    ...
    try:
        ...
        prompt = build_prompt(
            ...
            allow_precheck=allow_precheck, use_census=cfg.use_row_census_subagent,
        )
        options = build_options(
            cfg=cfg, run_id=run_id, workdir=workdir, schema=schema,
            expected_rows=exp_rows, use_census=cfg.use_row_census_subagent,
        )
        ...
        if structured is None and res.subtype in (
            ...
            opts2 = build_options(
                cfg=cfg, run_id=run_id, workdir=workdir, schema=schema,
                expected_rows=exp_rows, use_census=False, resume=res.session_id,
            )
            opts2.max_turns = 2
            opts2.max_budget_usd = (res.cost_usd + 0.15)
        ...
        structured = normalize_envelope(structured, field_name)
        ...
        while rounds < cfg.max_repair_rounds and res.session_id:
            ...
            opts_r.max_turns = 6
            opts_r.max_budget_usd = res.cost_usd + 0.25
            ...
            if do_rows:
                if missing:
                    ...
                    # No anchor columns: the rows can't be named, only counted.
            ...
            if validate_envelope(repaired, field_name, schema):
                break
            repaired = normalize_envelope(repaired, field_name)
...
# ═════════════════════════════════════════════════════════════════════════════
# 10. HARNESS OUTPUT (same layout/format as eval/studies/agent_baseline)
# ═════════════════════════════════════════════════════════════════════════════
COST_FIELDS = [
    "paper", "field_name", "status", "subtype", "stop_reason", "n_rows",
    "plan_rows", "rows_missing", "rows_withdrawn",
    "repair_rounds", "quotes_checked", "quotes_failed_inloop",
    "cells_failed_grounding", "cells_from_table", "cells_from_prose", "cells_nr",
    "cells_value_anchored", "num_turns", "total_cost_usd", "input_tokens",
    "output_tokens", "cache_read_tokens", "cache_creation_tokens",
    "duration_ms", "session_id", "tool_calls", "trace_path", "notes", "error",
]
...
def rows_for_sheet(res: ExtractResult, columns: List[str]) -> List[Dict[str, str]]:
    """One CSV row per table row (level-2 sheet), matching the baseline harness."""
    env = (res.envelope or {}).get(res.field_name, {})
    rows = env.get("value")
    if not isinstance(rows, list):
        return []
    out = []
    for row in rows:
        ...
        rec = {"Paper": res.paper}
        for c in columns:
            rec[c] = _cell(row.get(c))
        out.append(rec)
    return out


def write_outputs(
    results: List[ExtractResult], field_def: Dict[str, Any], out_dir: Path
) -> None:
    out_dir.mkdir(parents=True, exist_ok=True)
    (out_dir / "raw").mkdir(exist_ok=True)
    cols = [c["field_name"] for c in field_def.get("subform_fields") or []]
    fname = field_def["name"]

    sheet_rows: List[Dict[str, str]] = []
    for r in sorted(results, key=lambda x: x.paper):
        sheet_rows.extend(rows_for_sheet(r, cols))
        if r.envelope is not None:
            (out_dir / "raw" / f"{r.paper}.json").write_text(
                json.dumps(r.envelope, indent=2, ensure_ascii=False)
            )

    with open(out_dir / f"{fname}.csv", "w", newline="") as f:
        w = csv.DictWriter(f, fieldnames=["Paper"] + cols, quoting=csv.QUOTE_ALL)
        w.writeheader()
        w.writerows(sheet_rows)

    with open(out_dir / "cost_report.csv", "w", newline="") as f:
        w = csv.DictWriter(f, fieldnames=COST_FIELDS)
        w.writeheader()
        for r in results:
            d = asdict(r)
            d["total_cost_usd"] = round(r.cost_usd, 6)
            w.writerow({k: d.get(k) for k in COST_FIELDS})
    ...
    summary = {
        ...
        "mean_cost_usd": round(
            sum(r.cost_usd for r in results) / max(len(results), 1), 4
        ),
        "mean_rows": round(sum(r.n_rows for r in results) / max(len(results), 1), 2),
        "max_rows": max([r.n_rows for r in results] or [0]),
        "mean_turns": round(sum(r.num_turns for r in results) / max(len(results), 1), 2),
        "rows_total": sum(r.n_rows for r in results),
        "cells_failed_grounding_total": sum(r.cells_failed_grounding for r in results),
        "row_mismatches": sum(1 for r in results if "ROW_MISMATCH" in r.notes),
        "rows_unresolved": sum(r.rows_missing for r in results),
        "rows_withdrawn": sum(r.rows_withdrawn for r in results),
        "row_repair_rounds": sum(
            1 for r in results if "row_repair_withdrew_rows" in r.notes
        ),
        # where the data actually lived. A high prose share is expected on this
        # corpus and is NOT a defect - it is why the extractor must not assume a
        # markdown table exists.
        "provenance": {
            "cells_from_table": sum(r.cells_from_table for r in results),
            "cells_from_prose": sum(r.cells_from_prose for r in results),
            "cells_nr": sum(r.cells_nr for r in results),
            "cells_value_anchored_fallback": sum(r.cells_value_anchored for r in results),
        },
...
# ═════════════════════════════════════════════════════════════════════════════
DEFAULT_SCHEMA = (
    _REPO / "eval/studies/ablation/base_schemas/"
    "dynamic_6c8eecce_ContinuousOutcomesV2.json"
)
...
def _load_env() -> None:
    try:
        ...
        return
    for p in (_REPO / "eval" / ".env", _BACKEND / ".env"):
        if p.exists():
            load_dotenv(p, override=False)


async def _main_async(args) -> int:
    schema_def = json.loads(Path(args.schema).read_text())
    sig_def, field_def = pick_table_field(schema_def)
    schema = build_output_schema(field_def, strict_nr_union=CFG.strict_nr_union_schema)

    papers = sorted(Path(args.markdown_dir).glob("*.md"))
    if args.papers:
        want = {p.strip().lower() for p in args.papers}
        papers = [p for p in papers if p.stem.lower() in want]
    if args.limit:
        papers = papers[: args.limit]

    print(f"schema      : {Path(args.schema).name}")
    print(f"signature   : {sig_def['class_name']}")
    print(f"table field : {field_def['name']}  "
          f"({len(field_def.get('subform_fields') or [])} cols, "
          f"{len(field_def.get('anchor_columns') or [])} anchors, "
          f"strategy={field_def.get('extraction_strategy')})")
    print(f"papers      : {len(papers)}   concurrency {args.concurrency}")
    print(f"model       : {CFG.model}   effort {CFG.effort_small}/{CFG.effort_large}")

    if args.dry_run:
        print("\n--- generated output_format json_schema ---")
        print(json.dumps(schema, indent=2)[:4000])
        if papers:
            md = papers[0].read_text(errors="replace")
            p = build_prompt("DRYRUN", papers[0].stem, md, sig_def, field_def,
                             schema, CFG, True, CFG.use_row_census_subagent)
            head = p[: p.find("<paper") ] + f"<paper>...{len(md)//4} tok...</paper>"
            print("\n--- prompt (paper elided) ---")
            print(head + p[p.find("RUN_ID:"):][:6000])
        return 0

    if not (os.environ.get("ANTHROPIC_API_KEY")
            or os.environ.get("CLAUDE_CODE_USE_BEDROCK")):
        sys.exit("ERROR: set ANTHROPIC_API_KEY (eval/.env) or CLAUDE_CODE_USE_BEDROCK=1")
    if not _HAVE_SOURCE_LINKER:
        print(f"WARNING: source_linker unavailable ({_SOURCE_LINKER_ERR}); "
              "grounding verification disabled", file=sys.stderr)

    sem = asyncio.Semaphore(args.concurrency)
    done = 0

    async def one(p: Path) -> ExtractResult:
        nonlocal done
        async with sem:
            r = await extract_table(
                paper_path=p, sig_def=sig_def, field_def=field_def, cfg=CFG,
                expected_rows=args.expected_rows,
                trace_dir=Path(args.out) / "traces",
            )
            done += 1
            print(f"  [{done}/{len(papers)}] {p.stem:24} {r.status:7} "
                  f"${r.cost_usd:.4f} {r.n_rows:2}rows(plan {r.plan_rows}) "
                  f"{r.num_turns}t {r.cells_failed_grounding}bad "
                  f"[tbl {r.cells_from_table}/prose {r.cells_from_prose}/NR {r.cells_nr}] "
                  + (f"! {r.error}" if r.error else "") + f" {r.notes}")
            if CFG.trace and args.show_trace and r.trace_path:
                print(render_trace(Path(r.trace_path)))
            return r

    t0 = time.time()
    results = await asyncio.gather(*(one(p) for p in papers))
    print(f"\nwall clock {time.time()-t0:.1f}s")
    write_outputs(list(results), field_def, Path(args.out))
    return 0
...
def main() -> int:
    ...
    ap.add_argument("--schema", default=str(DEFAULT_SCHEMA),
                    help="path to a compiled schema_def JSON")
    ap.add_argument("--markdown-dir", default=str(MARKDOWN_DIR))
    ap.add_argument("--papers", nargs="*", help='exact stems, e.g. "Artese 2015"')
    ...
    ap.add_argument("--expected-rows", type=int,
                    help="hint used to size effort/turns/budget (default: from col count)")
    ap.add_argument("--census", action="store_true",
                    help="enable the independent row-census subagent")
    ap.add_argument("--strict-nr-union", action="store_true",
                    help='schema uses oneOf["NR", array] instead of array + []')
    ap.add_argument("--trace", action="store_true",
                    help="write a per-session JSONL of thinking, tool calls, tool "
                         "results and per-step tokens to <out>/traces/; also "
                         "delivers reasoning summaries instead of discarding them")
    ap.add_argument("--show-trace", action="store_true",
                    help="also print each trace as a readable timeline (implies --trace)")
    ap.add_argument("--render-trace", metavar="PATH",
                    help="render an existing trace JSONL and exit")
    ap.add_argument("--dry-run", action="store_true")
    args = ap.parse_args()

    if args.render_trace:
        print(render_trace(Path(args.render_trace)))
        return 0

    CFG.model = args.model
    CFG.concurrency = args.concurrency
    CFG.use_row_census_subagent = args.census
    CFG.strict_nr_union_schema = args.strict_nr_union
    CFG.trace = args.trace or args.show_trace
    _load_env()
