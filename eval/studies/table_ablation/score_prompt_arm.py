"""Score the single-call PROMPT arm vs the production row_then_columns arm.

row_then_columns arm = production extraction (full_studies/<slug>/claude/...).
prompt arm           = the single-call CSV from run_prompt_arm.py.
Both scored with the SAME harness (forms.base_form.run_form + the project's scoring
registry, level2 sub-record matching) against the project's GT workbook.

Usage:
    python -m eval.studies.table_ablation.score_prompt_arm --form perio_interventions
    python -m eval.studies.table_ablation.score_prompt_arm --form abx_dichotomous_outcomes --no-llm
"""
...
from studies.table_ablation.forms_registry import FORMS

ARMS = ("row_then_columns", "prompt")
...
def _out_dir(spec) -> Path:
    return EVAL_ROOT / "outputs" / "table" / spec.slug / "claude"


def _arm_csv(spec, arm: str) -> Path:
    return spec.production_csv if arm == "row_then_columns" else spec.out_csv
...
def _score_arm(spec, arm: str, use_llm: bool) -> dict:
    ...
    result = run_form(
        ...
        canonical_form   = cfg.get("canonical_form"),
    )
...
def main() -> int:
    ...
    for arm in selected:
        s = by.get(arm)
        if s:
            print(f"  {arm:<18}{s.get('n_scored_fields','—'):>7}{n(s.get('macro_f1')):>9}"
                  f"{n(s.get('micro_f1')):>9}{n(s.get('macro_precision')):>7}{n(s.get('macro_recall')):>8}")
    if "row_then_columns" in by and "prompt" in by:
        r, p = by["row_then_columns"], by["prompt"]
        if isinstance(r.get("macro_f1"), (int, float)) and isinstance(p.get("macro_f1"), (int, float)):
            print(f"\n  Δ macro_f1 (row_then_columns − prompt) = {r['macro_f1'] - p['macro_f1']:+.3f}")
    print(f"\n  Full sheet: {out}")
    return 0
