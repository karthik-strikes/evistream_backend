...
if [ -f .env ]; then
    ...
    echo "ERROR: .env file not found. Run this script from the backend directory."
    exit 1
fi
...
celery -A app.workers.celery_app worker \
    ...
    --detach
sleep 1 && echo "[start_workers] pdf_worker started, PID=$(pgrep -f 'celery.*-n pdf_worker' | head -1)" >> logs/pdf_worker.log
...
celery -A app.workers.celery_app worker \
    ...
    --detach
sleep 1 && echo "[start_workers] codegen_worker started, PID=$(pgrep -f 'celery.*-n codegen_worker' | head -1)" >> logs/codegen_worker.log
...
celery -A app.workers.celery_app worker \
    ...
    --detach
sleep 1 && echo "[start_workers] extraction_worker started, PID=$(pgrep -f 'celery.*-n extraction_worker' | head -1)" >> logs/extraction_worker.log
